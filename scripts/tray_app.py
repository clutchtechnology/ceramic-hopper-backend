...
try:
    ...
    INFLUX_URL = os.getenv("INFLUX_URL", "http://localhost:8088")
...
STOP_TIMEOUT = 10
# 单例锁名称
SINGLE_INSTANCE_KEY = "hopper-backend-tray-app"
...
class TrayApp(QSystemTrayIcon):
    ...
    def _create_tray_icon(self, running: bool) -> QIcon:
        """创建托盘图标，根据服务状态显示不同颜色。"""
        # 1. 尝试从文件加载图标
        icon_path = WORKDIR / "assets" / "image" / "hopper.png"
        if icon_path.exists():
            # 加载原始图标
            ...
            if not original_pixmap.isNull():
                ...
                # 在圆形背景上绘制料仓图标
                icon_size = int(size * 0.8)  # 图标占背景的 60%
                icon_offset = (size - icon_size) // 2
                scaled_icon = original_pixmap.scaled(
                    icon_size, icon_size,
                    Qt.KeepAspectRatio,
                    Qt.SmoothTransformation
                )
                painter.drawPixmap(icon_offset, icon_offset, scaled_icon)
                painter.end()
                return QIcon(pixmap)
        
        # 2. 如果没有图标文件，使用动态绘制（备用方案）
        size = 64
        pixmap = QPixmap(size, size)
        pixmap.fill(Qt.transparent)
        painter = QPainter(pixmap)
    ...
    def notify_text(self, text: str) -> None:
        self.showMessage("料仓监控系统", text, QSystemTrayIcon.Information, 5000)
