# ============================================================
# 文件说明: parser_vib_db6.py - 振动传感器数据解析器 (DB6)
# ============================================================
# 专门用于解析 DB6 振动传感器数据
# 从 DB6 中提取 V(速度)/D(位移)/HZ(频率) 三组核心数据
# 输出与原 DB4 vibration 模块兼容的字段名 (VX/VY/VZ/DX/DY/DZ/HZX/HZY/HZZ)
# 字段定义内联在 config_hopper_vib_db6.yaml 中，不依赖 plc_modules.yaml
# ============================================================
...
# DB6 字段名 -> 输出字段名 映射
# 将 DB6 config 中的 vel/dis_f/freq 字段映射为与 DB4 兼容的 VX/DX/HZX 命名
_FIELD_MAPPING = {
    # vel 模块 (速度) -> VX/VY/VZ
    "vel_x": "VX",
    "vel_y": "VY",
...
class VibDB6Parser:
    ...
    从 DB6 中提取三组核心振动数据，合并为一个 vibration 模块输出:
    - vel (Offset 12-17): 速度幅值 -> VX/VY/VZ
    - dis_f (Offset 26-31): 位移幅值 -> DX/DY/DZ
    - freq (Offset 32-37): 频率 -> HZX/HZY/HZZ
    ...
    def __init__(self, config_path: str = None):
        ...
        Args:
            config_path: DB6 配置文件路径
        """
        self.config_path = Path(config_path) if config_path else self.PROJECT_ROOT / "configs" / "config_hopper_vib_db6.yaml"
        self.config = None
        self.load_config()
    ...
    def load_config(self):
        ...
        try:
            if not self.config_path.exists():
                print(f"[Parser] DB6 配置文件不存在: {self.config_path}")
                return
                
            with open(self.config_path, 'r', encoding='utf-8') as f:
                self.config = yaml.safe_load(f)
            
            db_num = self.config.get('db_number', 6)
            size = self.config.get('total_size', 38)
            print(f"[Parser] VibDB6Parser 初始化完成: DB{db_num}, 总大小{size}字节")
        except Exception as e:
            print(f"[Parser] VibDB6Parser 加载配置失败: {e}")
