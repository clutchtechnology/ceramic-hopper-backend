...
def load_db_config():
    """加载 DB8 配置"""
    config_path = Path("configs/config_hoppers.yaml")
    with open(config_path, 'r', encoding='utf-8') as f:
...
def process_device(data: bytes, device: dict, modules: dict, weight_history: dict):
    """处理单个设备"""
    ...
    for module in device['modules']:
        ...
        print(f"\n   [{module_tag}] {module_type} (偏移: {module_offset}, 大小: {module_size})")
        ...
        for fname, finfo in parsed.items():
            raw_str = f"{finfo['raw']:.2f}" if isinstance(finfo['raw'], float) else str(finfo['raw'])
        ...
        if module_type in CONVERTER_MAP:
            ...
            if module_type == 'WeighSensor':
                ...
                converted = converter.convert(converter_input, previous_weight=previous_weight, interval=5.0)
                weight_history[cache_key] = converted.get('weight', 0)
            else:
...
def main():
    ...
    try:
        ...
        print("# 短料仓 (4个)")
        print("#" * 70)
        for device in db_config['short_hoppers']:
            fields = process_device(data, device, modules, weight_history)
        ...
        print("# 无料仓 (2个)")
        print("#" * 70)
        for device in db_config['no_hoppers']:
            fields = process_device(data, device, modules, weight_history)
            all_fields[device['device_id']] = fields
        ...
        print("# 长料仓 (3个)")
        print("#" * 70)
        for device in db_config['long_hoppers']:
            fields = process_device(data, device, modules, weight_history)
            all_fields[device['device_id']] = fields
        ...
        for device_id, fields in all_fields.items():
            ...
            for module_tag, module_fields in fields.items():
                print(f"   [{module_tag}] {module_fields}")
